        ...
        <form method='post' action=''>
            <div class="row content">
                <div class="col-sm-9 text-left">
                    <span class="textTitle">Review Questions that requires answer</span>
                    <hr>
                <?php
                    $subjID ='';
                    //$studentID = '1';
                    ...
                    $sqlscript = "call generateQuestions(,'$c_topicid','$c_subjid','$c_topicid',$itemseasy,$itemsmoderate,$itemsdifficult,0,'$c_topicid',$totalitems)";
...
if(isset($_POST['qtopic'])) {
 ...
 $result = $con->query($sql);
     ...
     //while($row = $result->fetch_assoc()) 
    {
        ...
        $level = $row["level"];
        echo '<div class="questionLevel">'.$level.'</div>';
        echo '<div class="qaQuestions">';
        echo   "<h4>" .($a + 1).'. '.$questions. "</h4>";
        echo '<br>';
        echo '<div>';
            echo '<input id="question-1-answers-A" type="radio" name="question_answers" value="a"';
            if(isset($_POST['question_answers']) && $_POST['question_answers'] == 'a') {
                echo ' checked="checked"';   
            }
            echo " >";
            echo '<label for="question-1-answers-A"><span><span></span></span>'. $opt1 .'</label>';
        echo '</div>';
        echo '<div>';
            echo '<input id="question-1-answers-B" type="radio" name="question_answers" value="b"';
            if(isset($_POST['question_answers']) && $_POST['question_answers'] == 'b') {
                echo ' checked="checked"';   
            }
            echo " >";
            echo '<label for="question-1-answers-B"><span><span></span></span>'. $opt2 .'</label>';
        echo '</div>';
        echo '<div>';
            echo '<input id="question-1-answers-C" type="radio" name="question_answers" value="c"';
            if(isset($_POST['question_answers']) && $_POST['question_answers'] == 'c') {
                echo ' checked="checked"';   
            }
            echo " >";
            echo '<label for="question-1-answers-C"><span><span></span></span>'. $opt3 .'</label>';
        echo '</div>';
        echo '<div>';
            echo '<input id="question-1-answers-D" type="radio" name="question_answers" value="d"';
            if(isset($_POST['question_answers']) && $_POST['question_answers'] == 'd') {
                echo ' checked="checked"';   
            }
            echo " >";
            echo '<label for="question-1-answers-D"><span><span></span></span>'. $opt4 .'</label>';
        echo '</div></div>';
        echo "<input type='hidden' value='$topicID2' name='topicid'>";
    ...
    echo '<div class="col-xs-12 col-sm-12 col-lg-12 text-left">';
    echo '<input type="submit" name="sbmit" class ="btnQa" value="Submit Answer">';
    echo '<input type="submit" name="next" class ="btnQa" value="Next Question">';
    echo '</div>';
 }
        $b=$a+1;
...
<?php
  ...
  global $con;
  $myQuery = "SELECT SUM(eq.minutes) AS MIN FROM examquestion eq INNER JOIN studentsession ss ON eq.id = ss.questionid WHERE ss.studentid = '$StudentID' AND eq.topicid = '$c_topicid' AND FIND_IN_SET(eq.SUBJID, '$c_subjid')";
...
function countdown() {
 ...
 countdown();
            ...
            <div class="col-sm-3 text-left qaInsideResult">.col-sm-6</div>
        </div>
        </form>
    </div>
